...
for algo in algos:
    file_pattern = f"{algo}_HalfCheetah-v4_seed_*_2nd/{algo}_*_merged.csv"
...
def process_and_plot(data_config, plot_type):
    ...
    if plot_type == 'separate':
        for algo, (iterations, mean_curve, std_curve) in all_results.items():
            ...
            plt.title(f"HalfCheetah-v4 Performance ({algo})", fontsize=14)
            plt.ylim(0)
            plt.xlim(0)
            plt.legend()
            plt.savefig(f"HalfCheetah-v4_algo_{algo}.jpg")
            plt.show()
