...
for dn in drop_nums:
    file_pattern = f"ARNACDD_HalfCheetah-v4_dropnum_{dn}_seed_*_Adam_3rd/{dn}_*_merged.csv"
...
def process_and_plot(data_config, plot_type):
    ...
    if plot_type == 'separate':
        for drop_num, (iterations, mean_curve, std_curve) in all_results.items():
            ...
            plt.title(f"HalfCheetah-v4 Performance (drop_num = {drop_num})", fontsize=14)
            plt.ylim(0)
            plt.xlim(0)
            plt.legend()
            plt.savefig(f"HalfCheetah-v4_dropnum_{drop_num}.jpg")
            plt.show()
        ...
        plt.xlabel("No. of Iterations", fontsize=12)
        plt.ylabel("Score", fontsize=12)
        plt.title("HalfCheetah-v4 Performance", fontsize=14)
        plt.ylim(0)
        plt.xlim(0)
        plt.legend()
